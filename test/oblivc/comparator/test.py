...
def input_prep():
    ...
    minval = 1e+20
    maxval = 4e+20
    rand_input = random.randint(minval, maxval)
    rand_input = rand_input * random.random()
    return int(rand_input)
    return round(rand_input, 5)
...
def main():
    ...
    # Dissect the result
    server_out = results["server"]
    client_out = results["client"]
